...
<div class="panel panel-default">
  <div class="panel-heading">Kredi Yükle</div>
  <div class="panel-body">
    <?php if (isset($_GET["link"])): ?>
      ...
      <?php if (rows($row) > 0): ?>
        ...
        <?php elseif ($row["odeme_slug"] == "papara"): ?>
          <?php $api = json_decode($row["odeme_resp"],true); ?>
          <div class="alert alert-success text-center">
            <strong>Papara Kodu:</strong>
            <h4><?php if (isset($api["papara"])): ?><?=$api["papara"]?><?php endif; ?></h4>
          </div>
        <?php else: ?>
          <form action="<?=str_ireplace("%URL%",URL,$row["odeme_url"])?>" method="get">
    ...
    <?php else: ?>
      ...
      <?php while($row = row($s)): ?>
        <?php if (rows($s) == 1): ?>
          <?php go(URL."/kredi/".$row["odeme_slug"]); ?>
        <?php else: ?>
          <?php $json = json_decode($row["odeme_resp"],true); ?>
          <a href="<?=URL?>/kredi/<?=$row["odeme_slug"]?>" class="btn btn-block btn-default btn-lg"><?=$json["baslik"]?></a>
        <?php endif; ?>
      <?php endwhile; ?>
    <?php endif; ?>
  </div>
