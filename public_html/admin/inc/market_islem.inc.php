...
<div class="block block-bordered">
  <div class="block-header bg-gray-lighter">Market İşlemler</div>
  <div class="table-responsive">
    <table class="table table-striped table-advance table-hover" style="margin-bottom: 0">
      ...
      <?php
        ...
        $ksayisi = rows(query("SELECT * FROM market"));
        $limit = 20;
        $ssayisi = ceil($ksayisi/$limit);
        $baslangic = $sayfa * $limit - $limit;
        if($ksayisi > $limit){
          $query = query("SELECT * FROM market INNER JOIN uyeler ON uyeler.uye_id=market.market_uye INNER JOIN urunler ON urunler.urun_id=market.market_urun_id INNER JOIN kategoriler_urun ON kategoriler_urun.kategori_id = urunler.urun_kategori INNER JOIN sunucular ON sunucular.sunucu_id = kategoriler_urun.kategori_sunucu ORDER BY market_id DESC LIMIT $baslangic,$limit");
        }else{
          $query = query("SELECT * FROM market INNER JOIN uyeler ON uyeler.uye_id=market.market_uye INNER JOIN urunler ON urunler.urun_id=market.market_urun_id INNER JOIN kategoriler_urun ON kategoriler_urun.kategori_id = urunler.urun_kategori INNER JOIN sunucular ON sunucular.sunucu_id = kategoriler_urun.kategori_sunucu ORDER BY market_id DESC");
        }
        if(rows($query) < 1){
          echo '<tr><td class="text-center" colspan="6">Market geçmişi bulunmamaktadır.</td></tr>';
        }else{
          while($uRow = row($query)){
        ?>
        <tr>
          <td class="text-center"><?php echo $uRow["market_id"]; ?></td>
          ...
          <td class="text-center"><?php echo $uRow["market_urun_gun"]; ?></td>
          <td class="text-center"><?=date("d-m-Y H:i",$uRow["market_tarih1"])?></td>
        </tr>
      <?php } } ?>
      </tbody>
    </table>
