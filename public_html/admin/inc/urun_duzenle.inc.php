<?php
    ...
    if(isset($_GET['uid']) && !empty($_GET['uid'])){
  		...
  		if(!$uid){
  			...
  			$varmi = query("SELECT * FROM urunler WHERE urun_id = '$uid'");
  			if(rows($varmi) < 1){
  				go(ADMIN_URL.'/?go=urunler');
  				exit;
  			}else{
...
<?php
    ...
    if(!$urun_adi || !$urun_fiyat || !$urun_kategori || !$urun_icon || !$urun_icon_id){
      ...
      if(rows($varmi) < 1){
        ...
        $insert = query("UPDATE urunler SET
          ...
          urun_komut_sil = '$urun_komut_sil' WHERE urun_id = '$uid'");
        if($insert){
          echo alert('<strong>Ürün</strong> başarıyla güncellendi. Yönlendiriliyorsunuz...','success');
  ...
  $query = query("SELECT * FROM urunler INNER JOIN kategoriler_urun ON kategoriler_urun.kategori_id = urunler.urun_kategori WHERE urun_id = '$uid'");
  $row = row($query);
?>
      ...
      <div class="block block-bordered">
        ...
        <div class="block-content">
          <div class="form-group">
            ...
            <input type="text" name="urun_adi" class="form-control" placeholder="Örnek: Normal Vip" value="<?=isset($urun_adi)?$urun_adi:$row["urun_adi"];?>">
          </div>
          <div class="form-group">
            <label>Ürün Fiyatı</label>
            <div class="input-group">
            <span class="input-group-addon"><i class="fa fa-fw fa-try"></i></span>
              <input type="number" name="urun_fiyat" class="form-control" placeholder="Örnek: 30" value="<?=isset($urun_fiyat)?$urun_fiyat:$row["urun_fiyat"];?>">
            </div>
          </div>
          <div class="form-group">
            ...
            <span class="input-group-addon"><i class="fa fa-fw fa-calendar"></i></span>
              <input type="number" min="0" name="urun_gun" class="form-control" placeholder="Örnek: 30" value="<?=isset($urun_gun)?$urun_gun:$row["urun_gun"];?>">
            </div>
            ...
            <?php
              ...
              if(rows($varmi) < 1){
                ...
                while($krow = row($varmi)){
                  $selected = ($krow['kategori_id'] == $row["urun_kategori"])?"selected":null;
                  echo '<option value="'.$krow['kategori_id'].'" '.$selected.'>'.ss($krow['kategori_adi']).' ['.ss($krow['sunucu_adi']).']</option>';
                }
              }
            ?>
            </select>
          ...
          <div class="form-group">
            ...
            <textarea name="urun_aciklama" id="summernote"><?=isset($urun_aciklama)?$urun_aciklama:$row["urun_aciklama"];?></textarea>
          </div>
          <hr>
          <div class="form-group row">
              <div class="col-xs-6"  style="padding-right: 7px;">
                  <div class="form-group">
                      ...
                      <input name="urun_resim" type="text" class="form-control" placeholder="Gözükecek resim." value="<?=isset($urun_resim)?$urun_resim:$row["urun_resim"];?>">
                  </div>
              </div>
              <div class="col-xs-6"  style="padding-left: 7px;">
                  <div class="form-group">
                      ...
                      <?php foreach ($items as $key): ?>
                          <?php $id = explode(":", $row['urun_icon_id']); ?>
                          <?php if ($key["text_type"] == $row['urun_icon'] AND $key["meta"] == $id[1]): ?>
                          ...
                          <?php endif; ?>
                      <?php endforeach; ?>
                      <input type="hidden" id="sunucu_icon_hidden" name="urun_icon" value="<?=$row['urun_icon']?>">
                      <input type="hidden" id="sunucu_icon_id" name="urun_icon_id" value="<?=$row['urun_icon_id']?>">
                  </div>
              </div>
          </div>
        </div>
        ...
        <div class="block block-bordered">
          <div class="block-content">
            <div class="form-group">
              ...
              <button type="submit" name="urun_duzenle" value="urun_duzenle" class="pull-right btn btn-success"><i class="fa fa-fw fa-check"></i> Kaydet</button>
            </div>
          ...
          <table id="komutlar" class="table table-striped table-advance table-hover" style="margin-bottom: 0">
            ...
            <tbody>
              <?php
                ...
                $urun_komut_sil = explode('-/',$row['urun_komut_sil']);
                for($i=0;$i <= count($urun_komut)-1;$i++){
              ?>
              <tr>
                <td><input type="text" name="urun_komut[]" class="form-control" value="<?php echo $urun_komut[$i]; ?>" /></td>
                <td><input type="text" name="urun_komut_sil[]" class="form-control" value="<?php echo $urun_komut_sil[$i]; ?>" /></td>
